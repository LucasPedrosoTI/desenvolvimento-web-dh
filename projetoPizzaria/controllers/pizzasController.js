...
module.exports = {
    ...
    search: (req, res) => {
        ...
        if (busca) {
            let result = pizzas.filter(p =>
                p.nome.toUpperCase().includes(busca.toUpperCase())
            );
    ...
    store: (req, res) => {
        ...
        ingredientes = ingredientes.map(
            // remover possíveis espaços de cada vírgula
            ing => ing.trim()
        );
        ...
        fs.writeFileSync(
            path.join("database", "Pizzas.json"),
    ...
    delete: (req, res) => {
        ...
        pizzas.splice(
            pizzas.findIndex(e => e.id == req.params.id),
            1
        );
